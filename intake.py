...
def discover(folder):
    """Every photo in `folder`, oldest first."""
    if not os.path.isdir(folder):
        raise FileNotFoundError(f"'{folder}' is not a directory")
    shots = [(photo_time(p), p) for p in
...
def main():
    ...
    print(f"{len(shots)} photos -> {len(groups)} product(s), splitting on gaps over {args.gap}s\n")
    for i, group in enumerate(groups, 1):
        sku = names.get(i, f"UNNAMED-{i:02d}")
        span = (group[-1][0] - group[0][0]).total_seconds()
        print(f"  {i:2d}. {sku:24s} {len(group)} photos  {group[0][0]:%H:%M:%S}+{span:.0f}s")
        for n, (_, path) in enumerate(group):
    ...
    print(f"\nNext: toycat analyze --all, then toycat build")
    return 0
